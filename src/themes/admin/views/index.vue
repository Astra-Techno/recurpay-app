<template>
    <!-- Screen loader -->
    <div>
        <div v-if="isLoading" class="fixed inset-0 bg-[#fafafa] dark:bg-[#060818] z-[1] grid place-content-center animate__animated">
...
watch(
    ...
    async () => {
        ...
        if (store.user.token && (!path || path === 'login')) {
        ...
        if (!store.user.token && store.guestPages.indexOf(path) === -1) {
            if (store.userPages.indexOf(path) === -1) {
                router.push('/login');
                return;
            }
