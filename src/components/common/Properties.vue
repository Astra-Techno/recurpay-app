			...
			<template #body="{ rows }">
				<div class="space-y-2">
					<div v-for="property in rows" :key="property.id" class="bg-white p-4 rounded shadow">
						...
						<div class="text-sm text-gray-600 mb-2" v-if="display != 'dashboard'">							
							<div class="flex">{{  property.address1 }}</div>
					...
					<div v-if="rows.length === 0" class="text-center text-gray-500">No properties to list.</div>
					<div v-if="rowContents?.total > 2 && pagination === false" class="text-center text-gray-500">
						<router-link :to="{ name: 'PropertiesList', params: { mode: (owned? 'owned' : 'rented') } }">
							<button class="text-xs w-full bg-blue-500 text-white px-3 py-1 rounded-full">View All</button>
						</router-link>
					</div>
				</div>
			</template>
