  ...
  <div class="space-y-4">
    ...
    <div class="flex justify-between items-center py-2">
      ...
      <router-link v-if="!grouped" :to="{ name: 'AddPayment', params: { property_id: propertyId } }">
        <button
          class="text-xs bg-blue-500 text-white px-4 py-2 rounded-full font-semibold shadow hover:bg-blue-600 transition">
          Add Payment
        </button>
      </router-link>
    </div>
    ...
    <list ref="rowContents" class="w-full" tmpl="custom" :data-url="dataUrl" :sortBy="'p.id'" :sortOrder="'desc'"
      :filter-toggle="false" :messages="{ empty: 'No Payments to List' }" :page-limit="pageLimit"
      ...
      <template #body="{ rows, loading }">
            ...
            <div v-for="(payments, groupTitle) in rows" :key="groupTitle"
              ...
              <div class="flex justify-between items-start pb-3 border-b">
                ...
                <router-link :to="{ name: 'AddPayment', params: { property_id: payments[0].property_id } }">
                  <button
                    class="text-xs bg-blue-500 text-white px-3 py-1 rounded-full font-semibold hover:bg-blue-600 transition">
                    Add
                  </button>
                </router-link>
              </div>
              <div class="space-y-3 pt-2">
                <PaymentCard v-for="payment in payments" :key="payment.id" :payment="payment" displayType="dashboard"
        ...
        <template v-else>
          <!-- Flat List -->
          <div class="space-y-4 animate-fade-in">
            <PaymentCard v-for="payment in rows" :key="payment.id" :payment="payment" displayType="dashboard"
              class="hover:scale-[1.01] transition-transform transform" />
          </div>
        </template>
